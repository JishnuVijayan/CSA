...
function PatternCaptcha({ onSolved, solved }: { onSolved: () => void; solved: boolean }) {
  const [question] = useState(() => {
    const patterns = [
      { text: 'What comes next? 2, 4, 6, 8, ?', answer: '10' },
      { text: 'Complete the sequence: A, C, E, G, ?', answer: 'I' },
      { text: 'What is missing? 5, 10, ?, 20, 25', answer: '15' },
      { text: 'Next in series: Mon, Tue, Wed, ?', answer: 'Thu' },
      { text: 'Complete: Red, Orange, Yellow, ?', answer: 'Green' }
    ];
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">
            Please solve this pattern to verify you are human:
          </label>
...
export default function VoterRegistrationPage() {
  ...
  if (!verified) {
    return <HumanVerification onVerified={() => setVerified(true)} captchaType="pattern" />;
  }
